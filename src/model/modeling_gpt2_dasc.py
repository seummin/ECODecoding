...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.clf_loss_weight=5
        self.direct_topp = 0
        
        self.attr_token_emb = nn.Embedding(config.vocab_size, config.proj_dim)
        self.attr_predictors = nn.ModuleList([nn.Linear(config.proj_dim,1) for _ in range(self.num_controls)])
        self.attr_projectors = nn.ParameterList([torch.empty((config.n_embd,config.proj_dim))for _ in range(self.num_controls)])
    ...
    def compute_clf_loss(self, style_id, attr_context_emb, controls, labels, notnull):
        ...
        classification_idxs = torch.ge(controls, 0) #크거나 같은
        if not torch.any(classification_idxs): #모두가 False 인 경우
            return classifier_losses, None, None, None
        attr_context_emb = attr_context_emb[classification_idxs]                    # [bs, seq_len, K] # [bs,seq_len,proj_dim]
        ...
        next_tokens = labels[classification_idxs]
        next_tokens[next_tokens==-100]=50256 # [bs, seq_len]
        ...
        next_token_clf_scores = torch.einsum('ijk,ijk->ij', attr_context_emb, next_tokens_distrib) # [bs,seq_len] 각 단어의 내적값
        # 해당 단어의 attr_emb와 hidden_state의 embd가 가까워지도록
    ...
    def forward(
        ...
        attr_context_embd = [hidden_states @ w for w in self.attr_projectors]
        ...
        if controls is not None and labels is not None:
            for i in range(self.num_controls):
                notnull = labels.ne(-100) # label 부분 True
                clf_loss, next_token_clf_scores, classifier_labels, notnull = self.compute_clf_loss(i, attr_context_embd[i], controls[:, i], labels, notnull)
        ...
        if self.clf_infer_weight != 0:
            top_values, top_indices = torch.topk(lm_log_probs, 30)
            
            if eco_decoding:
                logits = top_values[0, -1] 
                lse = torch.logsumexp(logits, dim=0)
                probs = torch.exp(logits - lse)
                entropy = lse - torch.dot(probs, logits)
                eps = 1e-12
                top_values = top_values * (1 + 1.0 / (entropy + eps))
                # top_values = top_values*(1+(1/cand_entropy))
            
            masked_logits=torch.full(lm_log_probs.shape,-100,dtype=torch.float32).to(lm_log_probs.device)
            masked_logits.scatter_(-1, top_indices, top_values)
            output_logits = masked_logits
            
            entropy = Categorical(probs = F.softmax(lm_logits, dim=2)).entropy().unsqueeze(-1).expand(-1,-1,self.config.vocab_size) # batch, seq_len, vocab  
            
            for i in range(self.num_controls):
                controls0 = controls[:, i].view(-1, 1, 1) # [bsz,1,1]
                if controls0[0][0]==1:
                    
                    
                    control_mask = torch.ge(controls0, 0)
                    clf_probs = torch.sigmoid(attr_context_embd[i] @ self.attr_token_emb.weight.T) # batch, seq_len, vocab
                    
                    clf_control_probs = clf_probs * controls0 # batch seq_len vocab
                    clf_log_probs = torch.log(clf_control_probs)
                    if eco_decoding:
                        eps = 1e-12

                        class_softmax_tmp = 1
                        class_logits = clf_control_probs[0, -1, top_indices[0, -1]] * class_softmax_tmp  # shape: [num_candidates]

                        # 후보 확률로 변환
                        class_ent_prob = torch.softmax(class_logits, dim=0)  # shape: [num_candidates]

                        # 엔트로피 계산: H = -Σ p * log p
                        class_entropy = -torch.sum(class_ent_prob * torch.log(class_ent_prob + eps))
                        output_logits += self.clf_infer_weight *(1+(1/class_entropy)) * control_mask * clf_log_probs
                    else:
                        output_logits += self.clf_infer_weight * control_mask * clf_log_probs
